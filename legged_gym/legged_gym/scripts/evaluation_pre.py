...
def _resolved_checkpoint_label(args, eval_cfg):
    checkpoint_override = getattr(eval_cfg, "checkpoint_label", None)
    if checkpoint_override is not None:
        return str(checkpoint_override)
    checkpoint_path = getattr(eval_cfg, "checkpoint_path", None)
    if checkpoint_path:
        match = re.search(r"model_(\d+)\.pt$", os.path.basename(checkpoint_path))
        if match:
            return match.group(1)
    return _checkpoint_label(args)
...
def _resolved_policy_type(args, eval_cfg):
...
def evaluate(args, eval_cfg):
    ...
    policy_type = _resolved_policy_type(args, eval_cfg)
    if policy_type == "depth":
    ...
    if checkpoint_path:
        ...
        resolved_log_pth = os.path.dirname(checkpoint_path)
    else:
    ...
    checkpoint = _resolved_checkpoint_label(args, eval_cfg)
    ...
    default_csv_path = os.path.join(output_dir, basename + ".csv")
    csv_path = getattr(eval_cfg, "episode_csv_path", default_csv_path)
    json_path = getattr(eval_cfg, "summary_json_path", os.path.join(output_dir, basename + ".json"))
...
def append_training_eval_outputs(eval_json_path, eval_csv_path, summary):
    ...
    with open(eval_json_path, "w") as f:
        json.dump({"evaluations": history}, f, indent=2)

    file_exists = os.path.exists(eval_csv_path)
    row = {
        "training_iteration": summary.get("training_iteration"),
        "checkpoint": summary.get("checkpoint"),
        "policy_type": summary.get("policy_type"),
        **summary["metrics"],
    }
    with open(eval_csv_path, "a", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=OVERALL_CSV_FIELDS)
        if not file_exists:
            writer.writeheader()
        writer.writerow(row)


def log_eval_summary_to_wandb(summary):
    if wandb.run is None:
        return
    metrics = {
        "Eval/episodes": summary["metrics"]["episodes"],
        "Eval/success_rate": summary["metrics"]["success_rate"],
        "Eval/fall_rate": summary["metrics"]["fall_rate"],
        "Eval/stuck_rate": summary["metrics"]["stuck_rate"],
        "Eval/mean_mxd": summary["metrics"]["mean_mxd"],
        "Eval/mean_normalized_waypoints": summary["metrics"]["mean_normalized_waypoints"],
        "Eval/mean_edge_violation": summary["metrics"]["mean_edge_violation"],
    }
    step = summary.get("training_iteration")
    if step is None:
        wandb.log(metrics)
    else:
...
def evaluate_training_checkpoint(
    ...
    eval_cfg = argparse.Namespace(
        ...
        terrain_set="all",
        terrain_names=",".join(active_terrain_names),
    ...
    append_training_eval_outputs(eval_json_path, eval_csv_path, summary)
    log_eval_summary_to_wandb(summary)
    return summary
